    ...
    match chan.get_mut(key) {
      Some(current_chan) => {
        if current_chan.contains(&id) {
          ...
          println!("removed connection [{}] from [{}], [{}] connections remain", id, key, current_chan.len());
        } else {
          println!("could not find connection [{}] for [{}] in [{}] existing connections", id, key, current_chan.len());
        }
      }
      None => {
        println!("tried to remove connection [{}] from [{}] which does not have any connections", id, key);
      }
    }
  }
  ...
  pub fn throw_channel_exception(&self, key: &str, exclude: &[&Uuid], msg: &ResponseMsg) {
    match &mut self.channels.read().expect("cannot lock channels for read").get(key) {
      Some(current_chan) => {
        ...
        let filtered: Vec<&Uuid> = current_chan.iter().filter(|c| {
          println!("{:?} / {} == {}", exclude, c, !exclude.contains(c));
          !exclude.contains(c)
        }).collect();

        println!("sending message [{:?}] to [{}] using [{}] of [{}] connections", msg, key, filtered.len(), current_chan_size);

        let _: Vec<_> = filtered.iter().map(|id| 
          match self.connections.read().expect("cannot lock connections for read").get(id) {
            Some(f) => {
              f.send_message(msg.clone())
            }
            None => {
