    ...
    .as_str().ok_or_else(|| {
      ...
      "set_size" => {
        ...
        let rows: u16 = u16::try_from(list[1].as_u64().ok_or_else(|| {
          eprintln!("first element must be an integer")
        })?)
        .map_err(|_| {
          eprintln!("set size rows out of range")
        })?;

        let cols: u16 = u16::try_from(list[2].as_u64().ok_or_else(|| {
          eprintln!("element 2 must be an integer")
        })?)
        .map_err(|_| {
          eprintln!("set size cols out of range")
        })?;
